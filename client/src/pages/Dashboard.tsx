    ...
    <div className="min-h-screen" style={{ background: 'var(--background)' }}>
      <nav className="navbar">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Admin</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-2 mb-8">
          {nav.map(n => (
            <Link key={n.path} to={n.path} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-xl transition-all" style={{ background: location.pathname === n.path ? 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)' : 'var(--surface)', color: location.pathname === n.path ? 'white' : 'var(--text-secondary)', border: `1px solid ${location.pathname === n.path ? 'transparent' : 'var(--border)'}` }}>
              {n.icon}
              {n.label}
            </Link>
          ))}
