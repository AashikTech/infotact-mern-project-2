      ...
      <div className="table-container">
        <table>
          ...
          <tbody>
            {products.map(p => (
              <tr key={p._id}>
                <td>{editId === p._id ? <input value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="input text-sm py-1" /> : <span className="font-medium">{p.name}</span>}</td>
                <td><span className="category-tag">{p.category}</span></td>
                <td>{editId === p._id ? <input type="number" value={editForm.price} onChange={e => setEditForm({...editForm, price: +e.target.value})} className="input text-sm py-1 w-24" /> : <span className="font-medium">${p.price.toFixed(2)}</span>}</td>
                <td>{editId === p._id ? <input type="number" value={editForm.stock} onChange={e => setEditForm({...editForm, stock: +e.target.value})} className="input text-sm py-1 w-20" /> : <span className={p.stock === 0 ? 'badge badge-danger' : ''}>{p.stock}</span>}</td>
                <td>
                  {editId === p._id ? (
                    <div className="flex gap-2"><button onClick={() => save(p._id)} className="btn btn-primary btn-sm">Save</button><button onClick={() => setEditId(null)} className="btn btn-secondary btn-sm">Cancel</button></div>
                  ) : (
                    <div className="flex gap-2"><button onClick={() => { setEditId(p._id); setEditForm({name:p.name,price:p.price,stock:p.stock}); }} className="btn btn-outline btn-sm">Edit</button><button onClick={() => del(p._id)} className="btn btn-danger btn-sm">Delete</button></div>
                  )}
                </td>
              </tr>
            ))}
